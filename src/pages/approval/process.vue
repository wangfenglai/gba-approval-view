        ...
        <div class="fui-toolbar">
          ...
          <div class="buttons">
            <el-button-group>
              ...
              <!--受理，补正-->
              <clerkBtn v-else-if="['ACCEPT','SUPPLEMENT'].indexOf(currentProcess.state)!=-1"
                        :getDataFromComponent="getDataFromComponent" :processBaseinfo="processBaseinfo"
                        :getFunctionFromProcess="getFunctionFromProcess"/>
              <!--审批，暂停-->
              <auditBtn v-else-if="['AUDIT','SUSPEND'].indexOf(currentProcess.state)!=-1"
                        :getDataFromComponent="getDataFromComponent" :processBaseinfo="processBaseinfo"
                        :getFunctionFromProcess="getFunctionFromProcess"/>
              <!--办结-->
              <finishBtn v-else-if="['WAITFINISH'].indexOf(currentProcess.state)!=-1"
                         ...
                         :getFunctionFromProcess="getFunctionFromProcess"/>
              <!--通用-->
              <currencyBtn v-else-if="['CURRENCY'].indexOf(currentProcess.state)!=-1"
                           :getDataFromComponent="getDataFromComponent" :processBaseinfo="processBaseinfo"
                           :getFunctionFromProcess="getFunctionFromProcess"/>
              <!--确定(有输出材料)-->
              <outputBtn v-else-if="['OUTPUT'].indexOf(currentProcess.state)!=-1"
                         :getDataFromComponent="getDataFromComponent" :processBaseinfo="processBaseinfo"
                         :getFunctionFromProcess="getFunctionFromProcess"/>
            </el-button-group>
          </div>
        ...
        <div style="width:100%;height:420px;overflow-y: auto;overflow-x: hidden;">
          <el-collapse v-model="activeNames">
            <el-collapse-item title="01 事项信息" name="1">
              <!--引入业务信息组件-->
              <matterchildinfo ref="matterChildInfoComponent" :matterChildInfo="matterChildInfo" :packageName="processBaseinfo.packageName" :limitByPromise="processBaseinfo.limitByPromise"></matterchildinfo>
            </el-collapse-item>
            <el-collapse-item
              :title="'02 办件信息  '+(processBaseinfo.serialNo==null?'':'('+'办件编号：'+processBaseinfo.serialNo+')')"
              name="2">
              <!--引入办件信息组件-->
              <Processbaseinfo ref="processFormInfoComponent" :state="currentProcess.state" :packageName="processBaseinfo.packageName"
                               :getDataFromComponent="getDataFromComponent" :elementVals="elementVals"
                               :matterChildInfo="matterChildInfo"></Processbaseinfo>
            </el-collapse-item>
            <el-collapse-item title="03 申报材料" name="3">
              <!--引入材料列表组件-->
              <Paperlist ref="paperListComponent" :state="currentProcess.state"
                         :getDataFromComponent="getDataFromComponent" :matterChildInfo="matterChildInfo"></Paperlist>
            </el-collapse-item>
          </el-collapse>
        </div>
      </el-col>
...
<script>
  ...
  import currencyBtn from './process/approvalbtn/currency'
  import outputBtn from './process/approvalbtn/output'
  ...
  export default {
    ...
    components: {
      Paperlist,
      Matterchildinfo,
      receiveBtn, clerkBtn, Processbaseinfo, auditBtn, finishBtn, currencyBtn, outputBtn
    },
    data() {
      return {
    ...
    methods: {
      ...
      loadMatterChildInfo(matterChildId) {
        ...
        this.$http.get(URL.getmatterchildinfo + '' + matterChildId).then((rs) => {
            //判断是否结果是否正常
            if (rs.data.success) {
              this.matterChildInfo = rs.data.content;
            }
          }
        )
      },
      ...
      initPage() {
        ...
        //获取事项id
        let matterChildId = query.matter_child_id;
        //获取is_suspend
        ...
        //获取事项信息包括材料信息
        matterChildId && this.loadMatterChildInfo(matterChildId);
        //加载办事信息
