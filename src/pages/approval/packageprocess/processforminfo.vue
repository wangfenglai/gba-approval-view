<template>
  <content>
    ...
    <div style="padding:10px;">
      ...
      <!--自定义表单-->
      <div v-for="form in customFormList" :key="form.name">
        ...
        <el-form v-if="form.tags.length>0" :inline="true" :rules="processFormInfoRules" :model="processFormInfo" label-width="300px" :ref="form.id+''" :key="form.id">
          <el-form-item v-for="item in form.tags" :label="item.propertyChineseName" :key="item.propertyName" :prop="item.propertyName">
            <!--输入框-->
            <el-input :disabled="isOutput?false:isReadOnly" size="small" v-if="item.type=='text'" v-model="processFormInfo[item.propertyName]" :key="item.propertyName"></el-input>
            <!--下拉框-->
            <el-select :disabled="isOutput?false:isReadOnly" size="small" v-if="item.type=='select'" v-model="processFormInfo[item.propertyName]" :key="item.propertyName" style="width:190px;">
              <el-option v-for="option in item.options" :key="option" :label="option" :value="option">
              </el-option>
            </el-select>
            <!--银行网点-->
            <el-cascader :disabled="isOutput?false:isReadOnly" :options="getBankInfo" v-model="processFormInfo[item.propertyName]" :key="item.propertyName" size="small" style="width:692px;" v-if="item.type=='datetime'" filterable clearable placeholder="请选择(支持搜索)"></el-cascader>
            <!--多选-->
            <el-checkbox-group :disabled="isOutput?false:isReadOnly" v-model="processFormInfo[item.propertyName]" :key="item.propertyName" size="small" style="width:692px;" v-if="item.type=='checkbox'">
...
export default {
  props: {
    ...
    elementVals: { type: Array, default: [], required: true },
    packageInfo: { type: Object, default: [], required: true }
  },
  data() {
    return {
      ...
      processFormInfoRules: {
        ...
        postPostcode: [
          { required: true, message: '请输入收件邮编', trigger: 'change' }
        ], //收件邮编
        postCustomer: [
          { required: true, message: '请输入收件人姓名', trigger: 'change' }
        ], //收件人
        postPhone: [
          { required: true, message: '请输入收件电话', trigger: 'change' }
        ] //收件电话
      },
  ...
  computed: {
    ...
    //判断自定义表单类型是否为输出表单
    isOutput() {
      return ['WAITFINISH'].indexOf(this.state) != -1
    },
    //判断是否只读状态
    isReadOnly() {
      return (
    ...
    customFormList() {
      ...
      //根据状态确定显示输入材料还是输出材料
      let name = this.isOutput
        ? ['outPutMatterPaperFormList']
        : ['matterPaperFormList']
      this.state == 'FINISH' &&
        (name = ['matterPaperFormList', 'outPutMatterPaperFormList'])
      let formArray = [] //记录表单信息
      let propertys = {} //记录表单参数名称
      let rules = {} //记录规则
      name.forEach(valueName => {
        this.packageInfo[valueName].forEach(value => {
          let formObj = {
            name:
    ...
    formObj() {
      ...
      for (var property in this.processFormInfo) {
        ...
        if (customPropertyName) {
          this.processFormInfo[property] &&
            elementVals.push(customPropertyName + ':' + data)
        } else {
