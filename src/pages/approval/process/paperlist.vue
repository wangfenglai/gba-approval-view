    ...
    <div>
      ...
      <el-table :data="paperTableData" stripe highlight-current-row style="width: 100%">
        ...
        <el-table-column prop="zz" header-align="center" align="center" label="纸质">
          <template slot-scope="scope">
            <i class="fa fa-file-text" @click="zzSelected($event)" v-bind:data-id="scope.row.id" v-if="scope.row.zz" v-bind:class="[scope.row.zzUpload?active_icon:'',isReadOnly?diabled_icon:cursor_pointer]"></i>
          </template>
        </el-table-column>
    ...
    <div>
      <el-dialog title="材料上传" size="small" :visible.sync="dzDialogShow" :before-close="updateDz">
        <el-button-group>
          ...
          <el-upload name="file" style="float: left" accept="file" :action="paperUploadUrl" :multiple=true :show-file-list=false :on-success="onSuccess" v-if="isOutput?!inputIsReadOnly:!isReadOnly">
            <el-button type="primary" size="small">
              <i class="fa fa-upload fa-lg"></i>&nbsp;选择文件上传</el-button>
          </el-upload>
          <el-button type="primary" size="small" v-if="fileTableData.length>0 && (isOutput?!inputIsReadOnly:!isReadOnly)" @click="deleteBtnClick()">
...
export default {
  props: {
    getDataFromComponent: { required: true },
    matterChildInfo: { required: true },
    state: { required: true }
  },
  data() {
    return {
  ...
  computed: {
    ...
    paperTableData() {
      ...
      name.forEach(nameValue => {
        let tempMatterPaperList = this.matterChildInfo[nameValue]
